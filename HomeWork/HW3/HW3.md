...
int f_pc = [
	...
	#Modified:When command is jxx(not jmp), if mispredicted branch chosen jump to valA
	M_icode == IJXX && E_ifun != UNCOND && M_Bch : M_valA;
...
int new_F_predPC = [
	...
	#Added:When command is jxx(not jmp), firstly do not jump
	M_icode == IJXX && E_ifun != UNCOND:f_valP
...
int new_E_valA = [
	...
	D_icode == IJXX && E_ifun != UNCOND:E_valC;
...
bool D_bubble =
	...
	(E_icode == IJXX && E_ifun != UNCOND && e_Bch) ||
	# Stalling at fetch while ret passes through pipeline
